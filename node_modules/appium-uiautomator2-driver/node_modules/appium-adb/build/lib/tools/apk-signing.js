...
async function signWithCustomCert(apk) {
    ...
    try {
        ...
        try {
            ...
            const jarsigner = node_path_1.default.resolve(await (0, helpers_js_1.getJavaHome)(), 'bin', `jarsigner${support_1.system.isWindows() ? '.exe' : ''}`);
            const fullCmd = [
                jarsigner,
                '-sigalg',
                'MD5withRSA',
...
async function checkApkCert(appPath, pkg, opts = {}) {
    ...
    if (node_path_1.default.extname(appPath) === helpers_js_1.APKS_EXTENSION) {
    ...
    const hashMatches = (apksignerOutput, expectedHashes) => {
        for (const [name, value] of lodash_1.default.toPairs(expectedHashes)) {
            if (value && new RegExp(`digest:\\s+${value}\\b`, 'i').test(apksignerOutput)) {
                logger_js_1.log.debug(`${name} hash did match for '${node_path_1.default.basename(actualAppPath)}'`);
                return true;
            }
        }
        return false;
    ...
    if (SIGNED_APPS_CACHE.has(appHash)) {
        logger_js_1.log.debug(`Using the previously cached signature entry for '${node_path_1.default.basename(actualAppPath)}'`);
...
async function getKeystoreHash() {
    ...
    const keytool = node_path_1.default.resolve(await (0, helpers_js_1.getJavaHome)(), 'bin', `keytool${support_1.system.isWindows() ? '.exe' : ''}`);
